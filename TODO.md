...
- `marimo--iframe-strips-newlines.md` → marimo: `mo.iframe` srcdoc newline stripping in static/session export (why the piston's `//`→`/* */` regex exists; still reproduces on 0.24.0; root cause located: `flatten_string` in `marimo/_output/utils.py` wrapping the whole iframe element).
...
- `marimo--islands-resize-iframe-undefined.md` → marimo: every `mo.iframe` island logs `ReferenceError: __resizeIframe is not defined`. Worked around: the helper (copied from marimo's app shell, `marimo/_static/index.html`) is defined in our pages via `include-in-header` in `docs/_quarto.yml`, guarded so an upstream definition wins; drop the shim once fixed upstream.
- `marimo--qmd-flavor-incompatible-with-quarto-marimo.md` → marimo: updated 2026-09-03 after testing on 0.5.0. The fence half of the drift is **fixed**: quarto-marimo 0.5.0 renders marimo's native ```` ```{marimo .python} ```` fences directly (verified, hide_code attribute included), so `examples_to_qmd.py` no longer rewrites fences. What remains: the exporter writes `header:` where the extension only reads `pyproject:` (verified: a `header:` dependency is absent from the render sandbox), and the exporter emits no `engine: marimo` line, without which 0.5.0 never claims the page. Both failures are silent (exit 0, zero islands), which the draft calls out; our `scripts/check_islands.py` post-render gate now catches it locally.
...
Decided 2026-09-03: `examples_to_qmd.py` keeps its build-time read-only code blocks rather than switching to `#| echo: true`. The native option renders client-side after hydration only, gets no Quarto syntax highlighting or copy button, and hidden cells would lose their code entirely (no `code-fold`). The docs generation now runs as the Quarto `pre-render:` hook with the generated `.qmd` gitignored, and `scripts/check_islands.py` fails any render where a marimo page came out with zero islands (the check the pre-render migration was gated on).
