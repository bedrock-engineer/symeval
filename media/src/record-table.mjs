...
// per-frame delay so viewers can read the result. node-canvas downscales, gifenc
// encodes.
...
const page = await b.newPage({ viewport: { width: 1200, height: 1200 }, deviceScaleFactor: 2 });
...
async function parkMouse() {
  await page.mouse.move(CLIP.x + CLIP.width + 60, 12);
}
