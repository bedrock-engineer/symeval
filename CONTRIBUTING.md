...
- `docs/getting-started.qmd` (and `docs/examples/*.qmd`) — the docs pages, also
  ...
  hook, and a `post-render:` check (`scripts/check_islands.py`) fails the
  build if a notebook page rendered with zero marimo islands.
...
- **Notebook pages** (from `symeval_mo.py` or `examples/*.py`): the `.qmd` is
  regenerated by the `pre-render:` hook on every render, so touching any
  `.qmd` (or re-running `uv run task docs_generate` after a notebook edit)
  refreshes it. The generator only rewrites files whose content changed, so
  the preview watcher does not loop.
